...
const ComicsList = (props) => {
	...
	async function constructURL(pageNo){
		try{
			const  result  = await axios.get('http://localhost:4000/comics/page/'+pageNo);
			return result.data;			
		  }
		  catch(e){
			return [];
	...
	async function setSearchURL(search){
		try{
			const result = await axios.get('http://localhost:4000/comics/search/'+search);
			return result.data;
			...
			<Grid item xs={12} sm={6} md={4} lg={3} xl={2} key={show.id}>
				<Card className={classes.card} variant='outlined'>
						...
						<Link to={`/comics/${show.id}`}>
							<img
								className={classes.media} src={show.thumbnail && show.thumbnail.path ? show.thumbnail.path+"."+show.thumbnail.extension  : noImage}
								title='show image'
								alt={show.title}
							/>
							...
							<CardContent>
                                ...
                                <p>
									Page Count : {show.pageCount ? show.pageCount : 0}
								</p>
                                <p>
									{show.description ? show.description : 'No Description'}
								</p>
							</CardContent>
						</Link>
	...
	if (search.loading)
		...
		return (
			<div>
				...
				<br />
				<br />
				<Link className='pagelink' onClick={increasePageNum}  to={`/comics/page/${pageNum+1}`} >
						Next
					</Link>
				<br/>
				<br/>
		...
		return (
			...
			<div>   
				...
				<Link className='pagelink' onClick={decreasePageNum} to={`/comics/page/${pageNum-1}`} >
						Previous
					</Link>
				 <Link className='pagelink' onClick={increasePageNum}  to={`/comics/page/${pageNum+1}`} >
						Next
